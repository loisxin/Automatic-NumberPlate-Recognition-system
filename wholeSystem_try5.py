...
def servoControl():
  ...
  try:
    while flag:
      ...
      imRec.right=240
      imRec.top=60
      screen.blit(imDisplay, imRec) # Combine Ball surface with workspace surface
      
    
      if((text=='KAU 3881') or (text=='FBR 1449')):
        display="Allow Pass"
        text_surface = my_font.render(display, True, WHITE)#display Left servo History coloum
        ...
        if ( not GPIO.input(26) ):
          ...
          print "IR sensor2!"
          t=0.00225
          T=t+0.02 # how to run counterclockwise 90 degree??
          frequency=1/T
          #p.ChangeFrequency(frequency)
          dc=(t/T)*1000000
          #p.ChangeDutyCycle(dc)
